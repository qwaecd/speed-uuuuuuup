                        ...
                        .then(
                                Commands.literal("rankinglist")
                                        ...
                                        .then(Commands.argument("racetrack_name", StringArgumentType.string())
                                                .executes(context -> getRankingList(context, StringArgumentType.getString(context, "racetrack_name")))
                                                .then(Commands.argument("page", IntegerArgumentType.integer(0))
                                                        .executes(context -> getRankingList(context, StringArgumentType.getString(context, "racetrack_name"), IntegerArgumentType.getInteger(context, "page")))
                                                )
                                        )
                        )
                        .then(Commands.literal("clearRankingList")
                                .requires(source -> source.hasPermission(4))
                                .then(Commands.argument("racetrack_name", StringArgumentType.string())
                                        .executes(context -> clearRankingList(context, StringArgumentType.getString(context, "racetrack_name")))
                                )
                        )
                        .then(
